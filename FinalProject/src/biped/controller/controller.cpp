...
Controller::control(const bool& fast_domain)
{
     ...
     */
    // TODO LAB 7 YOUR CODE HERE.
    IMUData BMX_IMU_data = sensor_->getIMUDataBMX160();
     ...
     */
    // TODO LAB 7 YOUR CODE HERE.
    updateActiveStatus(MPU_IMU_data);
    ...
    if (fast_domain)
    {
         ...
         */
        // TODO LAB 7 YOUR CODE HERE.
        pid_controller_attitude_y_.setState(MPU_IMU_data.attitude_y);
         ...
         */
        // TODO LAB 7 YOUR CODE HERE.
        pid_controller_attitude_y_.setErrorDifferential(MPU_IMU_data.angular_velocity_y);
    ...
    output_position_x = output_position_x_;
    output_attitude_y = output_attitude_y_;
     ...
     */
    // TODO LAB 7 YOUR CODE HERE.
    ActuationCommand new_output(true, true, true, motor_left_pwm, motor_right_pwm);

    if (!active_) {
        new_output.motor_left_pwm = 0;
        new_output.motor_right_pwm = 0;
        
        actuator_->actuate(new_output);
     ...
     */
    // TODO LAB 7 YOUR CODE HERE.
    bool motor_left = (motor_left_pwm > 0) ? true : false;
    bool motor_right = (motor_right_pwm > 0) ? true: false;
    motor_left_pwm = fabs(motor_left_pwm);
    motor_right_pwm = fabs(motor_right_pwm);
     ...
     */
    // TODO LAB 7 YOUR CODE HERE.
    if (motor_left_pwm > MotorParameter::pwm_max)
        motor_left_pwm = MotorParameter::pwm_max;
    if (motor_left_pwm < MotorParameter::pwm_min)
        motor_left_pwm = MotorParameter::pwm_min;

    if (motor_right_pwm > MotorParameter::pwm_max)
        motor_right_pwm = MotorParameter::pwm_max;
    if (motor_right_pwm < MotorParameter::pwm_min)
        motor_right_pwm = MotorParameter::pwm_min;

    new_output.motor_enable = true;
    new_output.motor_left_pwm = motor_left_pwm;
    new_output.motor_right_pwm = motor_right_pwm;
    new_output.motor_left_forward = motor_left;
    new_output.motor_right_forward = motor_right;
    // Serial(LogLevel::info) << "ML PWM " <<motor_left_pwm << " MR PWM " <<motor_right_pwm << " MLF " << motor_left << " MRF " << motor_right;
    // Serial(LogLevel::info) << "att y " <<MPU_IMU_data.attitude_y << " att z " <<BMX_IMU_data.attitude_z << " posx " << encoder_data.position_x;
    // Serial(LogLevel::info) << "ox " << output_position_x_ << " oy " << output_attitude_y_ << " oz " << output_attitude_z_;
    actuator_->actuate(new_output);
}
